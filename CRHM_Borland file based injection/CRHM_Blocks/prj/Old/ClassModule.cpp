...
__declspec(dllexport) string Fstrings[] = {"Observation", "W_to_MJ/Int", "MJ/Int_to_W", "Average", "Minimum", "Maximum",
...
// module can be in FunctListN without being in ReadListN.
  ...
  while (p < ReadListN->Count && ReadListN->Strings[p] == Name.c_str()) { //
...
// called by ClassModule::declreadobs INIT to add observation
  ...
  if((Global::DTmax - Global::DTmin)/Global::Freq + newVar->FileData->Dt1 > newVar->FileData->Dt2){
    CRHMException TExcept((String("Observation file '") + String(newVar->FileData->DataFileName.c_str()) + "' shorter than model run!").c_str(), TERMINATE);
    LogError(TExcept);
  }
