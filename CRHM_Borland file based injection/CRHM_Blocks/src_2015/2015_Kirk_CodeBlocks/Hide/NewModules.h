...
const long  *Use_Observations_As_Supplied;
...
const float *snowmeltD; // melt depth from upstream drift (m/d)
const float *hru_p;
...
float *Scaling;       // Handles ratio HRU Area/length (m) of HRU to adjust input and output flows.
...
class ClassSoilK : public ClassModule {
public:

ClassSoilK(string Name, String Version = "undefined", CRHM::LMODULE Lvl = CRHM::PROTO) : ClassModule(Name, Version, Lvl) {};
...
//const float *Sd_water_frac_init;
const float *contrib_frac_init;
...
float contrib_frac_subtract(void);
float contrib_frac_add(const float contrib_frac_slice, const float delta_Sd);
...
ClassSoilK* klone(string name) const;
};
...
// declared variables
float *infil;           // snow and ice melt redirected to infiltration.
float *cuminfil;        // cummulative snow and ice melt directed to infiltration.
float *snowinfil;       // not used. Satisfies module 'Netroute etc.
float *meltrunoff;      // snow melt runoff
float *cummeltrunoff;   // cumulative snow melt runoff
float *runoff;          // ice melt
float *cumrunoff;       // cumulative ice melt runoff
float *SWEmelt;         // SWE snowmelt
float *SWEmelt_delayed; // SWE snowmelt delayed
float *cumSWEmelt;      // cumulative SWE snowmelt runoff
...
const float *iceLag;
const float *SWE_to_infil_K;
const float *firn_to_infil_K;
const float *ice_to_infil_K;
const float *rain_to_infil_K;

const float *firn_h_init;
const float **firn_h_array_init;
const float *firn_dens_init;
const float **firn_dens_array_init;
const long *Elev_Adj_firn_surf;
...
ClassXG(string Name, String Version = "undefined", CRHM::LMODULE Lvl = CRHM::PROTO) : ClassModule(Name, Version, Lvl) {};
...
const long *inhibit_kw_ki_update;
const long *Option_Kurylyk_Xie;
...
float ClassXG::SWE_to_rho(float SWE);

typedef void (ClassXG:: *tthaw)();
tthaw pthaw;
...
ClassMod_Exec(string Name, String Version = "undefined", CRHM::LMODULE Lvl = CRHM::PROTO) : ClassModule(Name, Version, Lvl) {};
  ...
  typedef __declspec(dllimport) long LoadModuleTypeInit(const char *Grp, long GrpNo, long hru_cnt, const char *S_Vars, long VarsNo, float **V, const char *V_Pars, long ParsNo, float **P);
  typedef __declspec(dllimport) long LoadModuleTypeStep(double DTnow); // time step. Zero at end of run.
...
class ClassPCM : public ClassModule {
public:

ClassPCM(string Name, String Version = "undefined", CRHM::LMODULE Lvl = CRHM::PROTO) : ClassModule(Name, Version, Lvl) {};
...
// declared variables
float *total_basin_area;
float *Current_Volume;
float *outflow_volume;
float *final_vol;
float *max_slough_volume;
float *final_area;
float *max_slough_area;
float *delta_vol;
float *volfrac;
float *areafrac;
float *runofffrac;
...
const float **Max_A_N;
const float *Init_V;
const float **Init_V_N;
const float *ROF;
const long  *NumSloughs;
const long  *Connect;
const long  **Connect_N;

// variable inputs
const float *Precip_Var;
const float *Evap_Var;
...
ClassPCM* klone(string name) const;
...
float ClassPCM::wetland_drainage_area(float maxarea);
};
...
class ClassTest : public ClassModule {
public:

ClassTest(string Name, String Version = "undefined", CRHM::LMODULE Lvl = CRHM::PROTO) : ClassModule(Name, Version, Lvl) {};
...
virtual float FunctionA(const float *L, long hh);
virtual float FunctionB(const float *X, long hh);

ClassTest* klone(string name) const;
};

class ClassSWEslope : public ClassModule {
public:

ClassSWEslope(string Name, String Version = "undefined", CRHM::LMODULE Lvl = CRHM::PROTO) : ClassModule(Name, Version, Lvl, 100) {};
...
const long *wheretoSlope; // [nhru]
const long *order; // [nhru]

// variable put inputs
float *SWE;

// variable inputs - used to control loading position
...
Classquinton(string Name, String Version = "undefined", CRHM::LMODULE Lvl = CRHM::PROTO) : ClassModule(Name, Version, Lvl) {};
long nlay;
long nhru;
...
const float *slope;         // (m/m)
const float *length;        // length normal to creek
const float *Ka;            // empirical constant for K-d pow(a*d, b)
const float *Kb;            // empirical constant for K-d pow(a*d, b)
const float *Residual;      // Organic non-drainable porosity ()
const float **Residual_lay; // Organic non-drainable porosity ()
const float *n_const;       // Van Genutchen constant
const float **n_const_lay;  // Van Genutchen constant
...
Classqmelt(string Name, String Version = "undefined", CRHM::LMODULE Lvl = CRHM::PROTO) : ClassModule(Name, Version, Lvl) {};
long nhru;
...
class Classglacier_org : public ClassModule {
public:

Classglacier_org(string Name, String Version = "undefined", CRHM::LMODULE Lvl = CRHM::PROTO) : ClassModule(Name, Version, Lvl) {};
...
Classglacier_org* klone(string name) const;
};
...
class Classglacier2 : public ClassModule {
public:

Classglacier2(string Name, String Version = "undefined", CRHM::LMODULE Lvl = CRHM::PROTO) : ClassModule(Name, Version, Lvl){};
...
Classglacier2* klone(string name) const;
};
