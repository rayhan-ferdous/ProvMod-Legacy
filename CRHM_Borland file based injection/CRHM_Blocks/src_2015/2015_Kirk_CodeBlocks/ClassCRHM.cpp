...
//---------------------------------------------------------------------------
void ClassVar::Intvl(void){  // used for NFREQ observation reads
  float X;
...
//---------------------------------------------------------------------------
void ClassVar::LoopFirst(void){ // only first interval of day
...
//---------------------------------------------------------------------------
void ClassVar::LoopLast(void){ // only last interval of day
...
void ClassVar::Avg_(void){
  ...
  for(long ii = 0; ii < this->dim; ++ii) {
    float X = FunctVar->FileData->Data [FunctVar->offset+min<long>(Global::HRU_OBS[HRU_OBS_indexed][ii], FunctVar->cnt)-1] [Index_-FunctVar->FileData->IndxMin];
    values[ii] += X/Divisor;
  }
};

//---------------------------------------------------------------------------
void ClassVar::Max_(void){
...
//---------------------------------------------------------------------------
void ClassVar::Count_(void){

  for(long ii = 0; ii < this->dim; ++ii) {
    float X = FunctVar->FileData->Data [FunctVar->offset+min<long>(Global::HRU_OBS[HRU_OBS_indexed][ii], FunctVar->cnt)-1] [Index_-FunctVar->FileData->IndxMin];
    if(X > 0.0)
      values[ii] += 1.0/FunctVar->FileData->Freq;
  }
};

//---------------------------------------------------------------------------
void ClassVar::Count0_(void){
...
void ClassVar::do_p(ClassModule *thisModule){ // adjusts observation for lapse rate
  ...
  Classobs *thisObs = (Classobs *) thisModule;
  for(int ii = 0; ii < this->dim; ++ii) {
    values[ii] *= (Global::Warming_p[ii] * (1.0 + (Global::hru_elev[ii] - Global::obs_elev[ii])/100.0 *Global::ppt_adj[ii]));
  }
};
...
void ClassVar::do_t(ClassModule *thisModule){ // adjusts t_max and t_min observations for lapse rate and climate warming
  ...
  for(int ii = 0; ii < this->dim; ++ii) // does this step
    values[ii] -= Global::lapse_rate[ii]*(Global::hru_elev[ii] - Global::obs_elev[ii])/100.0 - Global::Warming_t[ii];
...
void ClassVar::do_t_day(ClassModule *thisModule){ // FUNCTION - adjusts observation for lapse rate and handles climate warming
    ...
    for(int ii = 0; ii < this->dim; ++ii) {
      float Fix = Global::lapse_rate[ii]*(Global::hru_elev[ii] - Global::obs_elev[ii])/100.0 - Global::Warming_t[ii];
      for(int tt = 0; tt < Global::Freq; ++tt){
  ...
  for(int ii = 0; ii < this->dim; ++ii) {
    for(int tt = 0; tt < Global::Freq; ++tt){
      if(Global::RH_VP_flag[ii]){ // maintain ea if possible
        Global::obs_ea[tt][ii] = estar(Global::obs_t_obs[tt][ii])*layvalues[tt][ii]/100.0; // use original t
        if(Global::obs_t_obs[tt][ii] > Global::obs_t[tt][ii]){ // new t lower, could be supersaturated
          float New_es = estar(Global::obs_t[tt][ii]); // maximum ea from new t
        ...
        Global::obs_ea[tt][ii] = estar(Global::obs_t[tt][ii])*layvalues[tt][ii]/100.0; // adjust ea
    }
  }
};
  ...
  for(int ii = 0; ii < this->dim; ++ii) {
    for(int tt = 0; tt < Global::Freq; ++tt){
      if(Global::RH_VP_flag2[ii]){ // maintain ea if possible
        Global::obs_ea[tt][ii] = estar(Global::obs_t[tt][ii])*layvalues[tt][ii]/100.0; // use new t
        if(Global::obs_t_obs[tt][ii] > Global::obs_t[tt][ii]){ // new t lower, could be supersaturated
          float New_es = estar(Global::obs_t[tt][ii]); // maximum ea from new t
          if(Global::obs_ea[tt][ii] > New_es){
            Global::obs_ea[tt][ii] = New_es;  // in saturation
...
void ClassVar::do_ea_day(ClassModule *thisModule){ // FUNCTION - ea available
  ...
  for(int ii = 0; ii < this->dim; ++ii) {
    for(int tt = 0; tt < Global::Freq; ++tt){
      Global::obs_rh[tt][ii] = layvalues[tt][ii]/estar(Global::obs_t_obs[tt][ii])*100.0; // use original t to calculate RH
      New_es = estar(Global::obs_t[tt][ii]); // maximum ea from new t
...
void ClassVar::do_ppt(ClassModule *thisModule){ // adjusts observation for lapse rate
  ...
  for(int ii = 0; ii < this->dim; ++ii) {
    values[ii] = values[ii]*(1.0 + (Global::hru_elev[ii] - Global::obs_elev[ii])/100.0 *Global::ppt_adj[ii]);
  }
};
...
//---------------------------------------------------------------------------
ClassPar::ClassPar(string module, string param, CRHM::TDim dimen, // Fix ???
      ...
      : module(module), basemodule(""), param(param), varType(varType), dimen(dimen), help(help),
        units(""), valstr(""), minVal(0), maxVal(0),
        values(NULL), ivalues(NULL), layvalues(NULL), ilayvalues(NULL), Identical(NULL), // string(NULL),
...
// allocate new memory
  ...
  if(varType == CRHM::Float) {
    if(lay > 0) {
      ...
      if(dimen != CRHM::NREB){ // NREB does not own memory
        for(int ii = 0; ii < lay; ii++)
...
ClassPar::ClassPar(string module, string param, CRHM::TDim dimen,
             ...
             : module(module), basemodule(""), param(param), varType(varType), dimen(dimen),
               valstr(valstr), minVal(minVal), maxVal(maxVal),
               help(help), units(units),
               values(NULL), ivalues(NULL),
               layvalues(NULL), ilayvalues(NULL), // ??? string(NULL),
...
ClassPar::ClassPar(ClassPar &p){  // copy constructor
  ...
  if(varType == CRHM::Float)
    ...
    for(long ii = 0; ii < p.Strings->Count; ++ii){
      Strings->AddObject(p.Strings->Strings[ii], p.Strings->Objects[ii]);
    }
  }
}
...
void ClassPar::Change(ClassPar &p){  // changes parameter data to 'p'
  ...
  if(varType == CRHM::Txt) {
   ...
   while(Strings->Count && Strings->Count < dim)  // duplicate last field when # of HRUs increased
     Strings->AddObject(p.Strings->Strings[Strings->Count-1], p.Strings->Objects[Strings->Count-1]);
  }
  else {
  ...
  for(itPar = Global::Mapdeclpar.begin(); itPar != Global::Mapdeclpar.end(); ++itPar){
    ...
    if(sparam == thisPar->param){
      ...
      if(itPar0 != Global::MapPars.end()) // already there?
        continue;
      if(Global::OurModulesList->IndexOf(smodule) == -1) // in group
        continue;
      ++Count;
...
ClassPar *ClassParFind(string module, string param){ // routine concatenates MODULE and NAME
  ...
  if((itPar = Global::MapPars.find("Shared " + param)) != Global::MapPars.end())
    ...
    long jj = Global::OldModuleName->IndexOf(module); // Check if renamed module. Find old name
    if(jj > -1){
...
ClassVar::ClassVar(string module, string name, CRHM::TDim dimen,
             ...
             : module(module), name(name), DLLName(""), root(""), varType(varType), lay(0), nfreq(false),
               ...
               UserFunct(NULL), FunctVar(NULL), CustomFunct(NULL), PointPlot(PointPlot), TchrtOpt(0),
               dimen(dimen) {
...
double ClassData::Encode24(int D[]){
  ...
  try {
    if(D[3] == 24) // handle 24:00
      return  EncodeDateTime(D[0], D[1], D[2], 0, D[4]) + 1;
    else
      return  EncodeDateTime(D[0], D[1], D[2], D[3], D[4]);
  }
  catch(...)
  {
//    string S;
...
/*          CRHMException Except("Observation file WARNING " + DataFileName, ERR);
        ...
        bool white = true;
        long Fix = -3;
        do {DataFile.seekg(Fix, ios::end);  // find last line in file
          --Fix;
          c = DataFile.get();
          if(white)
            white = isspace(c);
...
/*          if(Position != 0 && Times[Position] <= Times[Position-1]){
          ...
          for (int ii = 0; ii < DataCnt; ++ii) {
            ...
            while(DataFile.peek() == ' ') DataFile.get();

/*            if(DataFile.peek() == '\n') { // handles short line
              string S = string("Check line ") + string(HdrLen + Position + 1)
                           + string(" for missing columns in ");
              CRHMException Except(S + DataFileName, ERR);
              Message(Except.Message.c_str(),
                "Project observation file - too few columns", mbOK);
              LogError(Except);
              LineError = true;
              break;
            }*/
...
// Demote observations used by currently loaded modules
  ...
  for (itVar = Global::MapVars.begin(); itVar !=  Global::MapVars.end(); itVar++) {
    ...
    if(thisVar->FileData == this) {
      ...
      if(thisVar->DLLName.empty()) thisVar->cnt = 0;
      thisVar->offset = 0;
    }
  }
}
...
// calendar
  ...
  if(!strcmp(when, "start")){
    ...
    return  julian(Global::DTstart);
  }
  else if(!strcmp(when, "end")){

    return  julian(Global::DTend);
  }
  else{ // now

    return  julian(Global::DTnow);
  }
}
...
long MonthDaysCnt(TDateTime DT){
  ...
  DTNext = EncodeDateTime(Y, M, D, H, Min);
...
// calendar
  ...
  if(!strcmp(when, "start")){
    ...
    return  (float) julian(Global::DTstart);
  }
  else if(!strcmp(when, "end")){

    return  (float) julian(Global::DTend);
  }
  else{ // now

    return  (float) julian(Global::DTnow);
  }
}
...
void dattim(char *type, long *itime) { // dimension itime[6]
  ...
  if(type == "start") {
    DecodeDateTime(Global::DTstart, Year, Month, Day, Hour, Min);
  }
  else if(type == "end") {
    DecodeDateTime(Global::DTend, Year, Month, Day, Hour, Min);
   }
  else {
    DecodeDateTime(Global::DTnow, Year, Month, Day, Hour, Min);
  }
  itime[0] = Year;
  itime[1] = Month;
  itime[2] = Day;
...
void dattim(double DT, long *itime) { // dimension itime[6]
  ...
  DecodeDateTime(DT, Year, Month, Day, Hour, Min);
...
// executes following code only if observation file replaced
  if((itVar = Global::MapVars.find(s1 + " " + s2)) != Global::MapVars.end()) {
    ...
    Convert convert; convert.CheckUnitsObs(Comment, thisVar->units, name); // check original observation units

    thisVar->FileData = FileData;
    thisVar->cnt = cnt;
    thisVar->offset = offset;

    if(thisVar->dim != thisVar->cnt) {
  ...
  thisVar->varType =  CRHM::Read;
  if(Comment.empty())
    thisVar->help = "No information in observation file";
  else
...
//  Message(Mess2.c_str(), string("Error in filter: " + F + "\" in observation file: \"" + MyObs->DataFileName + "\"").c_str(), mbOK);
//  LogError(Except);
  Error = true;
}
...
void Classmacro::addfilter(string Line) {
  ...
  string S = Line.substr(1, trim(Line).length());
...
/*      CRHMException Except(Mess.c_str(), ERR);
  ...
  else if(Filter == "missinginter") {NewFilter = (Classfilter*) new ClassMissingInter(File, ToVar, FilterStuff);
    ++Interpolation;
  }
  ...
  else if(Filter == "sin") NewFilter = (Classfilter*) new Classsin(File, ToVar, FilterStuff);
...
/*      Message(MyObs->DataFileName.c_str(), "Missing data in first interval of filter 'missing'", mbOK);
...
void ClassMissingInter::doFunc(long Obs, long Line){
  ...
  if(FirstTime) {
    if((Data[0][Obs][Line] <= Constants[0] || Data[0][Obs][Line] >= Constants[1]) && !Warned) {
/*      Message(MyObs->DataFileName.c_str(), "Missing data in first interval of filter 'missinginter'", mbOK);
      LogError(string("Missing data in first interval of ") + MyObs->DataFileName.c_str(), WARNING);*/
      Warned = true;
      LastGoodData[Obs] = -1;
      return;
...
//---------------------------------------------------------------------------
void __fastcall LogMessage(const string S, float V, TExtra Opt){
  ...
  string SS = D + S + FloatToStrF(V, ffFixed, 7, 4);

  Global::LogScreen->Add(SS);
}

//---------------------------------------------------------------------------
void __fastcall LogMessage(const string S, float V,const string S2,  TExtra Opt){
  ...
  string SS = D + S + FloatToStrF(V, ffFixed, 7, 4) + " " + S2;

  Global::LogScreen->Add(SS);
}

//---------------------------------------------------------------------------
void __fastcall LogMessage(const string S, long V, TExtra Opt){
  ...
  string SS = D + S + FloatToStrF(V, ffFixed, 7, 0);;

  Global::LogScreen->Add(SS);
}

//---------------------------------------------------------------------------
void __fastcall LogMessage(const string S, TExtra Opt){
...
void __fastcall LogMessage(long hh, const char *S, float V, TExtra Opt){
  ...
  string A = string("HRU ") + longtoStr(hh+1) + ": ";
  string D;
  switch (Opt) {
    case BLANK:
    break;
  ...
  string SS = A + D + S + FloatToStrF(V, ffFixed, 7, 4);
...
void __fastcall LogMessage(long hh, const char *S, long V, TExtra Opt){
  ...
  string SS = A + D + S + FloatToStrF(V, ffFixed, 7, 0);
...
void __fastcall LogDebug(const string S){
  ...
  Global::LogScreen->Add(S);
}

//---------------------------------------------------------------------------
void __fastcall LogDebug(long h, const string Text, float v){

  string S = longtoStr(h+1);

  S += string(Text) + FloatToStrF(v, ffFixed, 7, 4);
...
//---------------------------------------------------------------------------
void __fastcall LogMessage(long hh, const string S, float V1, float V2, TExtra Opt){

  string SS = S + FloatToStrF(V1, ffFixed, 7, 4) + FloatToStrF(V2, ffFixed, 7, 4);
...
void __fastcall LogMessage(long hh, const char *S, float V1, float V2, float V3, TExtra Opt){
  ...
  string SS = S + FloatToStrF(V1, ffFixed, 7, 4) + " " + FloatToStrF(V2, ffFixed, 7, 4) + " " + FloatToStrF(V3, ffFixed, 7, 4);
  LogMessage(hh, SS.c_str(), Opt);
}
//---------------------------------------------------------------------------

void __fastcall LogMessage(long hh, const string S, float V1, float V2, float V3, TExtra Opt){

  string SS = S + FloatToStrF(V1, ffFixed, 7, 4) + " " + FloatToStrF(V2, ffFixed, 7, 4) + " " + FloatToStrF(V3, ffFixed, 10, 4);
  LogMessage(hh, SS.c_str(), Opt);
}

//---------------------------------------------------------------------------
void __fastcall LogMessageA(long hh, const string S, float V1, const float HRU_area, const float Basin_area, TExtra Opt){

  string SS = S + FloatToStrF(V1, ffFixed, 7, 2) + " " +
                  FloatToStrF(V1*HRU_area, ffFixed, 7, 2) + " " +
                  FloatToStrF(V1*HRU_area/Basin_area, ffFixed, 7, 3);
  LogMessage(hh, SS.c_str(), Opt);
}

//---------------------------------------------------------------------------
void __fastcall LogMessageA(long hh, const string S, float V1, const float Basin_area, TExtra Opt){

  string SS = S + FloatToStrF(V1, ffFixed, 7, 2) + " " +
                  FloatToStrF(V1/Basin_area, ffFixed, 7, 3);
  LogMessage(hh, SS.c_str(), Opt);
}
//---------------------------------------------------------------------------
void __fastcall LogMessageA(long hh, const string S, float V1, const float HRU_area, const float Basin_area, const string S2, TExtra Opt){

  string SS = S + FloatToStrF(V1, ffFixed, 7, 2) + " " +
                  FloatToStrF(V1*HRU_area, ffFixed, 7, 2) + " " +
                  FloatToStrF(V1*HRU_area/Basin_area, ffFixed, 7, 3) + " " + S2;
  LogMessage(hh, SS.c_str(), Opt);
}

//---------------------------------------------------------------------------
void __fastcall LogMessageA(long hh, const string S, float V1, const float Basin_area, const string S2, TExtra Opt){

  string SS = S + FloatToStrF(V1, ffFixed, 7, 2) + " " +
                  FloatToStrF(V1/Basin_area, ffFixed, 7, 3) + " " + S2;
  LogMessage(hh, SS.c_str(), Opt);
}
//---------------------------------------------------------------------------
...
Unit_Info Convert::Infos[] = {
    ...
    Unit_Info("µ", "Micron", 1e-6, "m") ,
    Unit_Info("Å", "Angstrom", 1e-10, "m") ,
    Unit_Info("", "", 0, "") };
...
string tcsinc(string pszIn){
  ...
  return pszIn.substr(0);
}

string tcsSkipSpaces(string pszIn ){
  while(pszIn[0] == ' ')
    pszIn = pszIn.substr(0);
  return pszIn;
}
...
void Convert::CheckUnitsObs(string Comment, string units, string where){ // check original observation units
  string::size_type Idx;
  double d;

  Comment = tcsSkipSpaces( Comment );
  if(Comment[0] == '('){
    Idx = units.find_first_of(")");
    if(Idx != string::npos){
      Comment = Comment.substr(0, Idx +2); // removes extra from obs files
      if(!ConvertUnit(d, tcsSqueeze(Comment), tcsSqueeze(units)) || d != 1.0) {
        CRHMException Except("Units error: '" + units + " instead of " + Comment + "' in "  + where, WARNING);
        LogError(Except);
      }
    }
  }
...
long julian_month(long Year, long julian) {
    int Y, Month, Day, H, M;
    TDateTime DT = EncodeDateTime(Year-1, 12, 31, H, M);

    DT += (double) julian;
    DecodeDateTime(DT, Y, Month, Day, H, M);
    return Month;
}
