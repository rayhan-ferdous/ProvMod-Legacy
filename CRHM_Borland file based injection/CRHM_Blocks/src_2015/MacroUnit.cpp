...
float Fday() { // used for variable DAY
  ...
  DecodeDateTime(Global::DTnow, year, month, day, h, m);
  return day;
}
...
void ClassMacro::decl(void) {
  ...
  if(isGroup || isStruct) {
    ...
    string S = longtoStr((max<long>(GroupCnt, StructCnt)));
    ...
    if(isStruct){
      ...
      declparam("HRU_struct", CRHM::NHRU, "[1]", "1", longtoStr(GrpStringList->Count), string("select using 1/2/3 ... module/group from '" + Choice + "'"), "()", &HRU_struct); // _group
    }
    ...
    for(int ii = 0; ii < GrpStringList->Count; ++ii){
      ...
      if(Indx == -1){
        Message(string("Module: '" + S + "' not found!"), "Macro Error");
...
ClassMacro::ClassMacro(string Name, int ThisBegin, string Version, string Desc): ClassModule(Name, Version, CRHM::MACRO){
  ...
  while(S = trim(Global::MacroModulesList->Strings[ThisEnd]), SS = S.substr(0,3),
          !(SS == "end" &&
  ...
  for(int ii = ThisBegin + 1; ii < ThisEnd - 1; ++ii) {
    ...
    DefCRHM::DefStringList->Strings[0] = trimleft(DefCRHM::DefStringList->Strings[0]);
    ...
    if(DefCRHM::DefStringList->Strings[0].find("command") == 0) {
    ...
    else if(!DefCRHM::DefStringList->Strings[0].compare("declgetparam") && Pcnt >= 4) {
      ...
      Message(string(string("function not recognised or too few parameters: ")
...
void Defdeclgetvar::CallDecl() {
  ...
  if(Global::BuildFlag == CRHM::INIT){ // moved down 06/20/11
    ...
    FP = Macro->vars.find(name);
    if(FP == Macro->vars.end()){
      Macro->vars.insert(make_pair(name, VarCHRM())); // just find name
      FP = Macro->vars.find(name);
    }
...
void Defdeclputvar::CallDecl() {
  ...
  Macro->FindWildVarFloat(name, thisVar); // just find name // just find name
