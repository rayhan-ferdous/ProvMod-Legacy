    ...
    case CRHM::INIT:  {
      ...
      if((itVar = Global::MapVars.find(source + " " + name)) != Global::MapVars.end()) {
        ...
        *value = newVar->values;
        ...
        GetUnit = FindWildVarFloat(name, newVar); // name against root
        if(GetUnit > -1){
          *value = newVar->values;
          if(layvalue != NULL){
            float ***deref = const_cast<float***> (layvalue);
        ...
        GetUnit = FindWildVarFloat(name, newVar); // name less group (if possible) against root
        if(GetUnit > -1){
          *value = newVar->values;
          if(layvalue != NULL){
            float ***deref = const_cast<float***> (layvalue);
...
//        const_cast<long **> (*layvalue) = newVar->ilayvalues; CHECK???
//        long ***deref = const_cast<long***> (layvalue);
//        *deref = newVar->ilayvalues;
//        *value = newVar->values;
  ...
  switch(Global::BuildFlag) {
    ...
    case CRHM::INIT:  {
      ...
      if((itVar = Global::MapVars.find(source + " " + name)) != Global::MapVars.end()) {
        newVar = (*itVar).second;
        *value = newVar->ivalues;
//        if(layvalue != NULL) const_cast<long **> (*layvalue) = newVar->ilayvalues; Check???
          if(layvalue != NULL){
            long ***deref = const_cast<long***> (layvalue);
            *deref = newVar->ilayvalues;
          }
