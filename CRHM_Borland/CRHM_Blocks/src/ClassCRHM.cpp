...
void ClassVar::Avg(void){
  ...
  for(ii = 0; ii < this->dim; ++ii) values[ii] = 0.0;

  do {
    for(ii = 0; ii < this->dim; ++ii) {
      X = FunctVar->FileData->Data [FunctVar->offset+min<long>(Global::HRU_OBS[HRU_OBS_indexed][ii], FunctVar->cnt)-1] [Index-FunctVar->FileData->IndxMin];
      values[ii] += X;
    }
    Count++;
...
void ClassVar::Max(void){
  ...
  for(ii = 0; ii < this->dim; ++ii) values[ii] = -100000.0;
...
void ClassVar::Tot_Ints(void){
  ...
  do {
    for(ii = 0; ii < this->dim; ++ii) {
      X = FunctVar->FileData->Data [FunctVar->offset+min<long>(Global::HRU_OBS[HRU_OBS_indexed][ii], FunctVar->cnt)-1] [Index-FunctVar->FileData->IndxMin];
      values[ii] += X/FunctVar->FileData->Freq;
    }
  } while (++Index%FunctVar->FileData->Freq != 0);
};
...
/*      CRHMException Except(Mess.c_str(), ERR);
  ...
  else if(Filter == "sine") NewFilter = (Classfilter*) new Classsine(File, ToVar, FilterStuff);
...
//---------------------------------------------------------------------------
Classsine::Classsine(ClassData *MyObs, string ToVar, string args, string argtypes) :
            Classfilter(MyObs, ToVar, args, argtypes) {
  readargs();
}

void Classsine::doFunc(long Obs, long Line){
