...
//---------------------------------------------------------------------------
void ClassVar::LoopFirst(long Option){
...
//---------------------------------------------------------------------------
void ClassVar::LoopLast(long Option){
...
//---------------------------------------------------------------------------
void ClassVar::LoopRange(long Option){
  ...
  switch(Option%8) {
    case 0:
      ...
      break;
    case 1:
      for(long ii = 0; ii < this->dim; ++ii) values[ii] = -1000000.0;
      break;
    case 2:
      for(long ii = 0; ii < this->dim; ++ii) values[ii] = 1000000.0;
      break;
  ...
  if(Option >= 8)
    for(long ii = 0; ii < this->dim; ++ii) // used by Count, Count0
      values[ii] = values[ii]/FunctVar->FileData->Freq;
}
...
void ClassVar::do_t(ClassModule *thisModule){ // adjusts t_max and t_min observations for lapse rate and climate warming
  ...
  for(int ii = 0; ii < this->dim; ++ii) // does this step
    values[ii] -= Global::lapse_rate[ii]*(Global::hru_elev[ii] - Global::obs_elev[ii])/100.0;
...
void ClassVar::do_p(ClassModule *thisModule){ // adjusts observation for lapse rate
  ...
  for(int ii = 0; ii < this->dim; ++ii) {
    values[ii] = FunctVar->values[ii]*(1.0 + (Global::hru_elev[ii] - Global::obs_elev[ii])/100.0 *Global::ppt_adj[ii]);
  }
};
...
void ClassVar::do_t_day(ClassModule *thisModule){ // FUNCTION - adjusts observation for lapse rate and handles climate warming
    for(int ii = 0; ii < this->dim; ++ii) {
      float Fix = Global::lapse_rate[ii]*(Global::hru_elev[ii] - Global::obs_elev[ii])/100.0;
      for(int tt = 0; tt < Global::Freq; ++tt){
...
//        Global::obs_ea[tt][ii] = New_es*Global::obs_rh[tt][ii]/100.0; // adjust ea
      }
    }
  }
};
...
//          Application->MessageBox(Except.Message.c_str(),"ONE line observation file!", MB_OK);
          LogError(Except);
          OneLine = true;
          Dt2 = Dt1 + 1.0;
          Freq = 1;
...
// Demote observations used by currently loaded modules
  ...
  for (itVar = Global::MapVars.begin(); itVar !=  Global::MapVars.end(); itVar++) {
    thisVar = (*itVar).second;
    if(thisVar->FileData == this && (thisVar->varType == CRHM::ReadF || thisVar->varType == CRHM::ReadI)) {
...
//  Comment = Comment.Trim();
  ...
  for (Cnt = 0; Cnt < argtypes.Length(); ++Cnt) { // allow for output location
    ...
    if(pp == string::npos) { // constant
      ...
      if(ObsCnt == 0 || thisVar->cnt < ObsCnt) // 04/17/13
        ObsCnt = thisVar->cnt;
    }
  }
  ...
  if(!ToVar.IsEmpty()) {
    ...
    outVar = declread("obs", ToVar.c_str(), ObsCnt, TotalCnt, MyObs, Comment.c_str()); // creates or re-uses
...
void Classmacro::addfilter(String Line) {
  ...
  else if(Filter == "rh") NewFilter = (Classfilter*) new Classrh(File, ToVar, FilterStuff);
  ...
  else if(Filter == "sine") NewFilter = (Classfilter*) new Classsin(File, ToVar, FilterStuff); // legacy
  else if(Filter == "sin") NewFilter = (Classfilter*) new Classsin(File, ToVar, FilterStuff);
  else if(Filter == "cos") NewFilter = (Classfilter*) new Classcos(File, ToVar, FilterStuff);
  else if(Filter == "abs") NewFilter = (Classfilter*) new Classabs(File, ToVar, FilterStuff);
  ...
  else if(Filter == "replace") NewFilter = (Classfilter*) new ClassReplace(File, ToVar, FilterStuff);
  else {
