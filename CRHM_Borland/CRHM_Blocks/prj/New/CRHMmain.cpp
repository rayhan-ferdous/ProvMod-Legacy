...
#define CurrentVersion "05/08/15"
...
String FStrings[] = {"Observation", "VP_saturated", "W_to_MJ/Int", "MJ/Int_to_W", "Average", "Minimum", "Maximum", "Daily Sum", "Positive",
                     "Total", "First", "Last", "Peak", "Count", "Count0", "Delta", "Intvl"};
//extern string Fstrings[];
...
const long colours[] = {clBlack, clDkGray, clBlue, clTeal, clFuchsia, clGreen, clNavy, clMaroon, clNavy, clOlive, clPurple, clRed, clTeal, clYellow, 0x4763FF, 0x7280FA, 0xFF00FF, 0x008CFF, 0x00FF7F, 0x20A5DA};
...
// module loop control
      ...
      if(Global::CRHMControlSaveCnt && !(Global::CRHMStatus & 1)){ // Set module mode. Save current position at end of day.
          ControlSaveState(Sender, false, StatePar, Global::ModuleBitSet);
...
// module loop control reset
      if((Global::CRHMStatus & 1) && (!Global::CRHMControlSaveCnt || Global::DTindx >= Global::DTmax-1)){ // handles daily + last day of run
        ControlReadState(Sender, false, NULL); // restore all to end of day. At top of loop incremented to start of next day.
...
// executes the DECL portion of the declvar/declparam etc. routines
  ...
  if(LogForm->DeclErrors > 0)
    Application->MessageBox("Module errors",
                               "Model Declaration Error", MB_OK);
  Label4Click(Sender);
}
//---------------------------------------------------------------------------
...
void __fastcall TMain::AddObsPlot(TObject *Sender, ClassVar *thisVar,
  ...
  double xx;
  long Option_; // used by functions
  double DTstartR = INT(DateTimePicker1->Date);
  double DTendR   = INT(DateTimePicker2->Date);
...
//    long Period = -1; // -1-all, 0-year and 1-month
    ...
    for (Global::DTindx = DTmin; Global::DTindx < DTmax; Global::DTindx+=Global::Freq) {
      ...
      if(Global::DTindx*Global::Freq/thisVar->FileData->Freq >= IndxMin
        ...
        (newVar->*(newVar->LoopFunct))(Option_);
    ...
    if(Funct == CRHM::DLTA){
      ...
      if(Season.size() > 1){
        Season[0].Xbefore = cdSeries->YValue[0];
        for(int ii = 1; ii < Season.size(); ++ii){
          long end = cdSeries->Count(); // covers last period when necessary
          if(Season.size()-1 > ii)
            end = Season[ii].Index;
          for(int nn = Season[ii-1].Index; nn < end; ++nn)
            cdSeries->YValue[nn] -= Season[ii-1].Xbefore; // subtract previous period end value
        }
      }
    }
...
void __fastcall TMain::FreeChart2(TObject *Sender){
      ...
      for (int ii = end; ii >= 0; --ii) { // reverse order because of the delete
          ...
          if(thisPar) {
            ...
            if(thisPar->varType == CRHM::Txt) {
              ...
              for(int jj = 0; jj <thisPar->lay; ++jj) {
                ...
                if(instr.fail() && newPar->lay-1 > jj) { //
                  for(int jjj = jj; jjj <thisPar->lay; ++jjj) {
                    for(int ii = 0; ii < newPar->dim; ++ii)
                      if(newPar->varType == CRHM::Float)
	...
	if(S[1] != '#') {
	  ...
	  SS = S.c_str();
          ...
          for(TBase = 0; TBase < 4; ++TBase)
            if(SS == TBases[TBase]){
                LabelBase->Caption = TBases[TBase] + MM;
              break;
            }
        }
      }
...
// all parameter values are set to shared values.  If re-defined in a module will be changed.
  ...
  if(PrjAutoExit->Checked && !PlotControl->IntervalControl && (All1->Checked || Last1->Checked || DebugScreen1->Checked)){
    int RUN_ID = 0;
    ClassPar *thisPar;
    thisPar = ParFind("basin RUN_ID");
    if(thisPar)
      RUN_ID = thisPar->ivalues[0];

    TRegistry *regkey = new TRegistry;
    regkey->RootKey = HKEY_CURRENT_USER;
    regkey->OpenKey("CRHM_output", true);
    regkey->WriteInteger("basin RUN_ID", RUN_ID);
    regkey->CloseKey();
...
{
  Funct = (TFun) RadioGroup1->ItemIndex;
  LabelFunct->Caption = RadioGroup1->Items->Strings[RadioGroup1->ItemIndex];
  RadioGroup1->Hide();
}
//---------------------------------------------------------------------------
...
CRHM::TFun  FindObservationType(String Kind){
  ...
  if(Kind == "_obs") Kind = "";
  for(int ii = CRHM::FOBS; ii <= CRHM::CNT0; ii++)
...
void __fastcall TMain::VarObsFunct_Update(TObject *Sender){
  ...
  for(int ii = 0; ii < ListBox4->Items->Count; ++ii) {
      ...
      if(Index == Main->Chart->SeriesCount()) // an observation
        continue;
...
// interrupt is not synced to end of day. 05/14/12
      ...
      cdSeries2->Clear();
      cdSeries2->ParentChart = NULL; // remove old series

      cdSeries->ParentChart = Chart; // remove old series from chart

      cdSeries->Tag = (int) thisVar2; // ClassVar*
  }  // for
}
//---------------------------------------------------------------------------
...
void __fastcall TMain::SummaryScreen1Rprt(TObject *Sender){
  ...
  bool *Wanted = new bool[Main->Chart->SeriesCount()];
  ...
  for(int ii = 0; ii < Main->Chart->SeriesCount(); ++ii) {
    if(Wanted[ii]){
      String S = Main->Chart->Series[ii]->Title;
      Sx += "\t" + S;
    }
  }
  LogList->Add(Sx);
  ...
  for(int nn = 0; nn < Main->Chart->Series[First]->Count(); ++nn){

    double Date = floor(Main->Chart->Series[First]->XValue[nn]);
    dattim(Date, itime);
    ...
    if(nn == Main->Chart->Series[First]->Count()-1){ // last time! Short of date by one.
      double Date = floor(2.0*(Main->Chart->Series[First]->XValue[nn]) - Main->Chart->Series[First]->XValue[nn-1]);
      dattim(Date, itime);
    ...
    if(Print){
      ...
      for (int vv = 0; vv < Main->Chart->SeriesCount(); ++vv) {
        if(Wanted[vv]){
