...
{
    result operator()(const T x, const T y) const
    {
        return (result) (x && y);
    }
};
...
{
    result operator()(const T Ht) const
    {
        return (result)  (101.3*pow((293.0-0.0065*Ht)/293.0, 5.26));    // kPa
    }
};
...
{
public:
    result operator() (const arg1  &x, const arg2 &x1) const
    {
        return (result) pow(x, x1);
    }
};
...
{
public:
    result operator() (const arg1  ea, const arg2 Pa) const
    {
        return (result) (0.622*ea/(Pa - ea*0.378));
    }
};
...
struct calculator : public grammar<calculator>
{
    ...
    struct definition
    {
        definition(calculator const& self)
        {
            ...
            ("while") ("endwhile")
            ("exp")("log")("max")("min")("sin")("cos") ("estar")
            ("log10")("patmos")("rhoa") ("tan");    // ("spec_humid")
            ...
            hydrology =
                ...
                |   ((chseq<>("patmos(") >> expression >> ')')  [make_op1(patmosC<const float>(), self.eval, self.oper)]) //                |   ((chseq<>("spec_humid(") >> expression >> *(ch_p(',')) >> expression >> ')')  [make_op2(spec_humidC<const float>(), self.eval, self.oper)]) //
