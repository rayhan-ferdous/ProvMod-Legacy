...
//          Application->MessageBox(Except.Message.c_str(),"ONE line observation file!", MB_OK);
        ...
        if(!OneLine)
        {
            ...
            do
            {
                DataFile.seekg(Back--);  // find last line in file
                c = DataFile.get();
                if(white)
                    white = isspace(c);
            }
...
//  Comment = Comment.Trim();
    ...
    if(!ToVar.IsEmpty())
    {
        ...
        if(ObsCnt == 0)
            ++ObsCnt; // handle case of constant
...
void Classmacro::addfilter(String Line)
{
    ...
    else if(Filter == "missinginter")
    {
        ...
        NewFilter = (Classfilter*) new Classsin(File, ToVar, FilterStuff); // legacy
    else if(Filter == "sin")
