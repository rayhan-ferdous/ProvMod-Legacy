...
void MoveModulesToGlobal(String DLLName){
  ...
  DLLModules.AddModule(new ClassSoil("Soil", "10/11/12", CRHM::BASIC));
...
/*     cos(x^s) = [(Slat * cos(Hr_Ang) * (-cos(ASL) * sin(GSL))
      ...
      for(int jj = 0; jj < CalcFreq; ++jj, Hr_Ang+=RADxxMIN){ // CalcFreq periods/day
...
// bubbling pressure = 1/alph
    ...
    for(int hhh = 0; hhh < hh || hhh == 0; ++hhh) { // includes first HRU
...
Classqdrift2* Classqdrift2::klone(string name) const{
  return new Classqdrift2(name);
}
...
//  Handle excess to interflow or runoff
    ...
    if(variation == VARIATION_1 && (soil_runoff[hh] > 0.0 || culvert_water_V[hh] > 0.0) && number_culverts[hh] > 0.0){ // culvert addition
      culvert_water_V[hh] += soil_runoff[hh]*hru_area[hh]*1000.0; // convert mm/m2 to m3
      soil_runoff[hh] = 0.0;
      ...
      if(culvert_water_H[hh] > culvert_water_Dmax[hh]){
        ...
        culvert_water_O[hh] = culvert_water_V[hh] - pow(culvert_water_Dmax[hh], 3.0)/(3.0*channel_slope[hh]*side_slope[hh]);
        soil_runoff[hh] = culvert_water_O[hh]/(hru_area[hh]*1000.0);
      }
      else
        culvert_water_O[hh] = 0.0;
      ...
      culvert_Q[hh] = (0.25781 *pow(HD, 4.0) - 1.52477*pow(HD, 3.0) + 2.79755*pow(HD, 2.0)
             - 0.521333*HD + 0.0619886)*pow(culvert_diam[hh], 2.5)*number_culverts[hh];

      if(culvert_water_V[hh] > culvert_Q[hh]*86400/Global::Freq)
        culvert_water_V[hh] -= culvert_Q[hh]*86400/Global::Freq; // m3/s to m3/interval
      else{
        culvert_Q[hh] = culvert_water_V[hh]*Global::Freq/86400;  // m3/interval to m3/s
        culvert_water_V[hh] = 0.0;
        culvert_water_A[hh] = 0.0;
        culvert_water_H[hh] = 0.0;
      }
      soil_runoff[hh] += culvert_Q[hh]/(hru_area[hh]*1000.0); // m3/interval to mm/m2
    } // culvert addition
    else
      culvert_Q[hh] = 0.0;
