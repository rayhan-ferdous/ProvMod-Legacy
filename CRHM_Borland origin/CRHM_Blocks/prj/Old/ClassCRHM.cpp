...
void ClassPar::Change(ClassPar &p){  // changes parameter data to 'p'
  ...
  if(varType == CRHM::Txt) {
   Strings->Assign(p.Strings);
   while(Strings->Count < dim)  // duplicate last field when # of HRUs increased
  ...
  for(itPar = Global::Mapdeclpar.begin(); itPar != Global::Mapdeclpar.end(); ++itPar){
    ...
    if(sparam == thisPar->param){
      ...
      if(itPar0 != Global::MapPars.end()) // already there?
        continue;
      if(Global::OurModulesList->IndexOf(smodule.c_str()) == -1) // in group
        continue;
      ++Count;
...
ClassPar *ClassParFind(string module, string param){ // routine concatenates MODULE and NAME
  ...
  if((itPar = Global::MapPars.find("Shared " + param)) != Global::MapPars.end())
    ...
    long jj = Global::OldModuleName->IndexOf(module.c_str()); // Check if renamed module. Find old name
    if(jj > -1){
      string s = Global::NewModuleName->Strings[jj].c_str(); // two step to remove \0
...
double ClassData::Encode24(int D[]){
  ...
  try {
    if(D[3] == 24) // handle 24:00
      return  EncodeTime((Word)0, (Word)D[4], (Word)0, (Word)0) +
              EncodeDate((Word)D[0], (Word)D[1], (Word)D[2]) + 1.0;
    else
      return  EncodeTime((Word)D[3], (Word)D[4], (Word)0, (Word)0) +
    ...
    if(!Simulation) {
      ...
      do{
        ...
        if(DataFile.eof()) { // one line file
          ...
          Application->MessageBox(Except.Message.c_str(),
          "ONE line observation file!", MB_OK);
          LogError(Except);
          OneLine = true;
          Dt2 = Dt1 + 1.0;
          Freq = 1;
      ...
      if(!OneLine) {
        ...
        bool white = true;
        do {DataFile.seekg(Back--);  // find last line in file
          c = DataFile.get();
          if(white)
            white = isspace(c);
        }
          ...
          if(Position != 0 && Times[Position] >= Times[Position-1] + DeltaH)
            if(!SparseFlag) {
              ...
              if(FirstFile) {
                ...
                Application->MessageBox(S.c_str(), string("Warning Sparse Observation File: " + DataFileName).c_str(), MB_OK);
              }
            }
...
// calendar
  ...
  if(!strcmp(when, "start")){
    ((TDateTime) Global::DTstart).DecodeDate(&Y,&M,&D);
    DtBegin = EncodeDate((Word)Y-1, (Word)12, (Word)31);
    return  (Global::DTstart - DtBegin);  // -0.999/Global::Freq
  }
  else if(!strcmp(when, "end")){
    ((TDateTime) Global::DTend).DecodeDate(&Y,&M,&D);
    DtBegin = EncodeDate((Word)Y-1, (Word)12, (Word)31);
    return  (Global::DTend - DtBegin);
  }
  else{ // now
    Global::DTnow.DecodeDate(&Y,&M,&D);
    DtBegin = EncodeDate((Word)Y-1, (Word)12, (Word)31);
...
//  ClassVar *thisVar;
  ...
  long ee = args.Pos(")");
  String Comment = args.SubString(ee+1, args.Length());
  Comment = Comment.Trim();
  if(Comment.IsEmpty())
    Comment = "filter";
  else{
    long tt;
    while(tt = Comment.Pos("\\t"), tt > 0){
      Comment.Delete(tt, 1);
      Comment[tt] = ' ';
    }
  }
  ...
  while(ee = args.Pos(','), ee > 0) args[ee] = ' ';
  ...
  if(!ToVar.IsEmpty()) {
    ...
    thisVar = declread("obs", ToVar.c_str(), ObsCnt, TotalCnt, MyObs, Comment.c_str()); // creates or re-uses
...
void Classmacro::addfilter(String Line) {
  ...
  if(Line.SubString(2, 1) == "$") return; // handle comment

  long bb = S.Pos('(');

  long ee;
  for(ee = 1; ee < bb; ++ee)
    if(S.IsDelimiter(" \t", ee)) break;
...
void __fastcall LogMessage(const char *S, float V, TExtra Opt){
  ...
  String SS = D + S + FloatToStrF(V, ffFixed, 10, 4);
