...
long julian(double Dttime){
  ...
  long Fix = (Dttime - Global::TdateTime_Offset)*86400.0;
...
tm timeinfo;
    ...
    time_t Current = mktime(&timeinfo), temp;

    return double(Current)/86400.0 + Global::TdateTime_Offset; // correction from
}
