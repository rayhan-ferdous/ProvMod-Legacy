...
string Fstrings[] = {"Observation", "W_to_MJ/Int", "MJ/Int_to_W", "Average", "Minimum", "Maximum",
                                             "Daily Sum", "Positive", "Total", "Total/Freq", "First", "Last", "Intvl"};

string AKAstrings[] = {"VARG", "OBSR", "VARD", "OBSD", "PARD", "OBSF"};
//---------------------------------------------------------------------------

void ClassModule::reset(void){
...
//---------------------------------------------------------------------------
ClassModule* ClassModule::klone() const {
...
TStringList* ClassModule::decldiagparam(string param, CRHM::TDim dimen,
  ...
  return declparam(param, dimen, Texts, help, StringsList, Local);
}
...
TStringList* ClassModule::declparam(string param, CRHM::TDim dimen,
  ...
  switch(Global::BuildFlag) {
    ...
    case CRHM::DECL:  {
      ...
      newPar = new ClassPar(Name, param, dimen, Texts, help, CRHM::Txt, nhru);
...
//  Convert convert; convert.CheckUnitsString(Name, param, units); // ???
  ...
  switch(Global::BuildFlag) {
    ...
    case CRHM::INIT:  {
       ...
       *value = newPar->values;
       if((dimen == CRHM::NLAY && layvalue != NULL) || (dimen == CRHM::NDEF && layvalue != NULL) || (dimen == CRHM::NDEFN && layvalue != NULL)){
         float ***deref = const_cast<float***> (layvalue);
         *deref = newPar->layvalues;
      }
       return;
      }
      else if((itPar = Global::MapPars.find("Shared " + param)) != Global::MapPars.end()) {
...
void ClassModule::declparam(string param, CRHM::TDim dimen,
  ...
  // convert; convert.CheckUnitsString(Name, param, units);
  ...
  switch(Global::BuildFlag) {
    ...
    case CRHM::INIT:  {
       ...
       *ivalue = newPar->ivalues;
       if((dimen == CRHM::NLAY && ilayvalue != NULL) || (dimen == CRHM::NDEF && ilayvalue != NULL) || (dimen == CRHM::NDEFN && ilayvalue != NULL)){
         long ***deref = const_cast<long***> (ilayvalue);
         *deref = newPar->ilayvalues;
       }
       return;
      }
      else if((itPar = Global::MapPars.find("Shared " + param)) != Global::MapPars.end()) {
  ...
  switch(Global::BuildFlag) {
    ...
    case CRHM::INIT:  {
      ...
      if((itVar = Global::MapVars.find(source + " " + name)) != Global::MapVars.end()) {
        ...
        *value = newVar->ivalues;
        if(layvalue != NULL){
          long ***deref = const_cast<long***> (layvalue);
          *deref = newVar->ilayvalues;
        }
...
long ClassModule::declobs(string name, CRHM::TDim dimen, string help, string units, float **value){
  ...
  // convert; convert.CheckUnitsString(Name, name, units);
    ...
    case CRHM::DECL:  {
      ...
      if((itVar = Global::MapVars.find(declModule + variable)) != Global::MapVars.end()) {
        ...
        if(dimen == CRHM::NHRU){
          ...
          if(newVar->FileData) // handles NO observations
            newVar->dim = newVar->cnt;
        }

         if(newVar->varType == CRHM::Read || newVar->varType == CRHM::ReadF) { // used only once
          ...
          if(newVar->varType == CRHM::Read || newVar->varType == CRHM::ReadF && nhru > newVar->dimMax) {
            if(dimen == CRHM::NHRU){ // NHRU
              newVar->dimMax = max<long>(newVar->dim, newVar->dimMax);
            }
            else { // NOBS
              newVar->dimMax = max<long>(max<long>(nhru, newVar->cnt), newVar->dimMax);
            }
...
/*            else{
              Convert convert;
              convert.CheckUnitsObs(newVar->help, units, variable); // check original observation units
            }*/
        ...
        else if(newVar->dimen != dimen && dimen == CRHM::NFREQ){ // upgrade t to t-freq or increase in dim
          newVar->ReleaseM(true);
          newVar->lay = Global::Freq;
          newVar->nfreq = true;
          newVar->layvalues = new float *[newVar->lay];
    ...
    case CRHM::INIT:  {
      ...
      if((itVar = Global::MapVars.find(declModule + variable)) != Global::MapVars.end()){ // look for in declModule
        ...
        if(newVar->varType == CRHM::Read || newVar->varType == CRHM::ReadF) { //  || newVar->varType == CRHM::Float || newVar->varType == CRHM::ReadF
          if(dimen == CRHM::NFREQ && newVar->lay == 0) {
            newVar->ReleaseM(true);
            newVar->lay = Global::Freq;
            newVar->nfreq = true;
            newVar->layvalues = new float *[newVar->lay];
          ...
          if(newVar->varType == CRHM::Read) { //  || newVar->varType == CRHM::Float|| newVar->varType == CRHM::ReadF
            //Convert convert; convert.CheckUnitsObs(newVar->help, units, variable); // check original observation units
            newVar->values = new float[newVar->dim];
      ...
      if(!newVar->cnt && newVar->DLLName.empty() && indx == string::npos) { //  && not declared module
        if(optional) {
//          LogError(CRHMException("Observation: '" + variable + "', not in Data file. Requested by '" + Name + "' (optional).", WARNING));
          *value = NULL;
          ...
          CRHMException Except("Observation: '" + variable + "', not in Data file. Requested by '" + Name + "'", TERMINATE);
          LogError(Except);
          throw Except;
        }
      }
...
long ClassModule::declobsfunc(string obs, string variable, float **value, CRHM::TFun typeFun, float ***layvalue, bool optional){
  ...
  switch(Global::BuildFlag) {
    ...
    case CRHM::DECL:  {
      ...
      if(!obsVar->values){
        obsVar->values = new float[nhru]; // TB 10/08/10 changed 04/20/11
        obsVar->varType = CRHM::ReadF;
      }
    ...
    case CRHM::INIT:  {
      ...
      if((itVar = Global::MapVars.find(declModule + obs)) != Global::MapVars.end()) {
        ...
        if(obsVar->FileData == NULL) {
          if(optional) {
//            LogError(CRHMException("Function observation: '" + variable + "', cannot be generated from the declared observation, '" + obs.c_str()+ "'. Requested by '" + Name + "' (optional).", WARNING));
            *value = NULL;
            return(-1);
          }
          else {
            CRHMException Except("Function observation: '" + variable + "', cannot be generated from the declared observation, '" + obs.c_str()+ "'. Requested by '" + Name + "'", TERMINATE);
            LogError(Except);
            throw Except;
          }
        }
        ...
        if(obsVar->FileData->Times != NULL) {
          CRHMException Except("Function observation: '" + variable + "', uses a 'sparse' Data file. Requested by '" + Name + "'", TERMINATE);
          LogError(Except);
          throw Except;
        }
...
// module can be in FunctListN without being in ReadListN.
  ...
  if(Global::DTindx%Global::Freq == 0) { // only at start of day
    while (pN != FunctListN->Count && FunctListN->Strings[pN] == Name) {
      ClassVar *P = (ClassVar*) FunctListN->Objects[pN];
      P->dim = nhru; // check if necessary. Group?

      if(LastGroup != this->GroupCnt){
        if(this->isGroup)
          ((ClassMacro*) this)->ObsModule->pre_run();
        else
          this->pre_run();
        LastGroup = this->GroupCnt;
      }

      if(P->FunctVar->FileData->GoodDay)
        (P->*(P->UserFunct))();

      if(P->CustomFunct && !P->nfreq) // execute any extra features except for nfreq observations
        (P->*(P->CustomFunct))(this); // do_t_day, do_rh_day, do_ea_day, do_t, do_p or Nothing

      ++pN;
    }
  }

  while (p < ReadListN->Count && ReadListN->Strings[p] == Name) { //
...
//  Global::HRU_OBS = Main->Local_Tables; // always correct? Not set by macro project?
  ...
  while (p < ReadListN->Count) {
    ...
    if(newVar == P){
      if(this->Name == this->NameRoot) // simple project
        return;
      else if(ReadListN->Strings[p] == this->Name) // group
        return;
    }
    p++;
  }

  if((Global::DTmax - Global::DTmin)/Global::Freq + newVar->FileData->Dt1 > newVar->FileData->Dt2){
    CRHMException TExcept((string("Observation file '") + string(newVar->FileData->DataFileName) + "' shorter than model run!"), TERMINATE);
    LogError(TExcept);
  }

  if(!GroupCnt)
    ReadListN->AddObject(newVar->module, (TObject*) newVar);
  else
    ReadListN->AddObject(Name, (TObject*) newVar);

  if(NameRoot == "obs" || Name == "obs"){ // handle lapse rate and climate warming

    if(newVar->name == "t_max" || newVar->name == "t_min") // newVar->name == "t" ||
      newVar->CustomFunct = &ClassVar::do_t;
    else if(newVar->name == "p")
...
// called by ClassModule::declobsfunc INIT to add observation function
  ...
  FunctListN->AddObject(Name, (TObject*) newVar);
  ...
  if(NameRoot == "obs" || Name == "obs"){ // handle vapour pressure
    ...
    else if(RootName == "RHday" && Global::RH_EA_flag > -1){
      newVar->CustomFunct = &ClassVar::do_rh_day;
      Global::RH_EA_flag = 1;
    }
    else if(RootName == "EAday" && Global::RH_EA_flag < 1){
      newVar->CustomFunct = &ClassVar::do_ea_day;
      Global::RH_EA_flag = -1;
    }
    else if(RootName == "t_min" || newVar->name == "t_max") // daily - t_min/t_max/t    || newVar->name == "t" || newVar->FunctVar->name == "t"
      newVar->CustomFunct = &ClassVar::do_t;
...
void ClassClark::ChangeLag(const float newlag, const long hh)
{
...
//---------------------------------------------------------------------------
void ClassModule::declputparam(string source, string param, string units, float **value, float ***layvalue){
...
//---------------------------------------------------------------------------
void ClassModule::declputparam(string source, string param, string units, long **ivalue, long ***ilayvalue){
