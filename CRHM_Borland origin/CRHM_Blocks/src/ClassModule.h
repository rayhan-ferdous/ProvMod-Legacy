...
string DLLName; // CRHM, Macro or DLL name // string
string ID; // Class name. If macro is the macro name
Administer *OurAdmin;
string Version; // string
long PeerRank;
string PeerVar; // string
CRHM::LMODULE Lvl;  // module level
...
ClassModule(string Name, string Version, CRHM::LMODULE Lvl = CRHM::PROTO, long PeerRank = 1000, string PeerVar = "") // string
  : Name(Name), NameRoot(Name), Version(Version), Lvl(Lvl), PeerRank(PeerRank), PeerVar(PeerVar), HRU_struct(NULL), isGroup(false),  isStruct(false),
      nhru(0), nobs(0), nlay(0), hh(0), variation(0), variation_set(0), variation_max(0),
      DLLName(""), GroupCnt(0), StructCnt(0), ModuleIndx(0), ID(typeid(*this).name()) {Dummy = -9999;};
...
virtual ClassModule* klone() const;
...
string Var_name(ClassModule* thisModule, string S); // string
...
int declgrpvar(string variable, string queryvar, string help, string units, float **value, float ***layvalue, bool PointPlot = false);
...
void declparam(string param, CRHM::TDim dimen,
               ...
               string help, string units, float const **value,
...
long declreadobs(string variable, CRHM::TDim dimen, string help, string units,
           const float **value, long HRU_index = 0, bool optional = false, const float ***layvalue = NULL);
...
void declputparam(string source, string name, string units, float **value, float ***layvalue = NULL);

void declputparam(string source, string name, string units, long **value, long ***layvalue = NULL);
...
long FindWildVarFloat(string name, ClassVar* &newVar, bool Org, bool Root);
