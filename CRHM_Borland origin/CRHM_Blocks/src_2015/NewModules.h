...
float Sd_add(const float applied_depth);
float Sd_subtract(const float applied_depth);

float area_frac(const float vol_frac);
...
float *SWE;
float *topo_elev;
float *hru_elev_value;
float *snowmeltD_to_net_rain;
float *cumsnowmeltD_to_net_rain;
long *nfirn;
...
ClassPCM(string Name, string Version = "undefined", CRHM::LMODULE Lvl = CRHM::PROTO) : ClassModule(Name, Version, Lvl, 1000, " hru_evap, , evap_Var") {};
...
float areafraction(float maxarea, float volume, float S, float h);
void calc_h_and_S(float maxvolume, float maxarea, float &S, float &h);
float wetland_drainage_area(float maxarea);
};
...
ClassSWEslope(string Name, string Version = "undefined", CRHM::LMODULE Lvl = CRHM::PROTO) : ClassModule(Name, Version, Lvl, 1000, "glacier_All, snowmeltD") {};
...
const float *snowmeltD; // melt depth from upstream drift
const float *hru_p;

// declared parameters
const long *DrainTo;        // drain to HRU
const long *Snowmelt_DrainTo;   // drain to HRU
...
float *Zdt;            // melt depth (m) To drive tsurface
...
Classqmelt2(string Name = "Qmelt2", string Version = "undefined", CRHM::LMODULE Lvl = CRHM::PROTO) : ClassModule(Name, Version, Lvl){};

// input variables
const float *hru_t;
const float *hru_tmean;
const float *hru_tsfD;
...
class Classobs_basic : public ClassModule {
public:

Classobs_basic(string Name, string Version = "undefined", CRHM::LMODULE Lvl = CRHM::PROTO) : ClassModule(Name, Version, Lvl) {};
...
Classobs_basic* klone(string name) const;
};

class Classquinton3 : public ClassModule {
public:

Classquinton3(string Name = "Quinton", string Version = "undefined", CRHM::LMODULE Lvl = CRHM::PROTO) : ClassModule(Name, Version, Lvl){};
...
const float **a_const_lay;  // Van Genutchen constant
const float *Drained;       // depth of surface drained organic material (m)
const float *FrostTable;    // initial depth of frost table (m)
const float *runoffstorage; // runoff parameter ()
const float *runoffLag;     // runoff parameter ()
const float *flowstorage;   // flow parameter ()
const float *flowLag;       // flow parameter ()
...
Classquinton3* klone(string name) const;
};
