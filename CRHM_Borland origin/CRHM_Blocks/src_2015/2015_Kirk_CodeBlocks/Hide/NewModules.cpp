...
void MoveModulesToGlobal(String DLLName){
  ...
  DLLModules.AddModule(new Classglacier("glacier", "06/02/15", CRHM::PROTO));
...
void Classobs::decl(void) {
  ...
  if(Global::BuildFlag == CRHM::INIT){ // required here to set up data for ClassModule::ReadObs
    ...
    Global::OBS_AS_IS = const_cast<long > (Use_Observations_As_Supplied[0]); // Can be done - applies to all
...
void Classobs::pre_run(void) {
  ...
  Global::RH_EA_obs = const_cast<long > (Using_RH_EA_obs);
}
...
void Classfrozen::run(void) {
  ...
  for(hh = 0; chkStruct(); ++hh) {
    ...
    if(nstep == 0){ // end of day
      ...
      CRHMException TExcept(String(String("Ice lens formed in HRU ") + String(hh+1) +", at minimum temperature of " + FloatToStrF(hru_tmin[hh], ffGeneral, 4, 1)).c_str(), WARNING);
      LogError(TExcept);
    }
    ...
    if(nstep == 1 && hh == 0){
      if(t0_Julian[0] == julian("now") || t0_Julian[0] == 0 && SWE_sum/nhru > 10 && snowmeltD[hh] > 2.0 && !Julian_lockout[0]){
        ...
        if(t0[0] <= 0)
          ...
          for(int hhh = 0; chkStruct(hhh); ++hhh) // save t0 values
            t0_Var[hhh] = t0[hhh];
    ...
    if(snowmelt > 0.0){ // snowmeltD[hh]/Global::Freq
      ...
      if(hh == (nhru-1) && (SWE_sum/nhru <= 2.0 || last_timestep())){ // end of melt
        if(SetOpportunityTime){ // was calculating opportunity time
          ...
          for(int hhh = 0; chkStruct(hhh); ++hhh){ // save opportunity time values
      ...
      if(snowmeltD[hh] > 2.0){
        ...
        if(Update_infil){
          for(int hhh = 0; chkStruct(hhh); ++hhh){ // set infiltration
...
// heights must be positive
	if (z0 <= 0 || zq <= z0 || zu <= z0 || za <= z0) {
                String D = Global::DTnow.FormatString(" yy mm dd hh nn ") + "hle1: height not positive ";
                String SS = D + "hru " + hh+1;
...
// pressures must be positive
	if (ea <= 0 || es <= 0 || press <= 0 || ea >= press || es >= press) {
                ...
                String SS = D + "hh " + hh +" 'hle1' " + ". H: " + FloatToStrF(h, ffFixed, 10, 4) + " le: " + FloatToStrF(xlh * e, ffFixed, 10, 4);
                SS = SS + " ta: " + FloatToStrF(ta-CRHM_constants::Tm, ffFixed, 10, 4) +
...
// latent heat flux (- away from surf)
  ...
  if(iter >= ITMAX){
    ...
    String Flag = "iters" + String(iter);
    LastDTnow = Global:: DTnow;

    String D = Global::DTnow.FormatString(" yy mm dd hh nn ");
    String SS = D + "hh " + hh +" 'hle1' " + Flag + ". H: " + FloatToStrF(h, ffFixed, 10, 4) + " le: " + FloatToStrF(xlh * e, ffFixed, 10, 4);
    SS = SS + " ta: " + FloatToStrF(ta-CRHM_constants::Tm, ffFixed, 10, 4) +
                 ", ts: " + FloatToStrF(ts-CRHM_constants::Tm, ffFixed, 10, 4) +
                 ", ea: " + FloatToStrF(ea, ffFixed, 10, 4) +
                 ", es: " + FloatToStrF(es, ffFixed, 10, 4) +
...
void ClassNetroute_M::init(void) {
  ...
  for(hh = 0; hh < nhru; ++hh){
    if(Ktravel[hh] >= (Global::Interval/(2.0*route_X_M[hh]))){
      String S = string("'" + Name + " (Netroute_M) Muskingum coefficient negative in HRU ").c_str() + IntToStr(hh+1);
      CRHMException TExcept(S.c_str(), WARNING);
      LogError(TExcept);
    }
...
void ClassNetroute_D::run(void) {
  ...
  for(hh = 0; chkStruct(hh); ++hh) {
    ...
    if(distrib_sum[hh] > 0.0){ // does not apply to last HRU
      for(int hhh = 0; chkStruct(hhh); ++hhh){
...
void Classglacier::decl(void) {
  ...
  declparam("Elev_Adj_firn_surf", NHRU, "[0]", "0","1", "1 - enable add depth of glacier to land elevation (hru_elev)", "()", &Elev_Adj_firn_surf);
  ...
  for(hh = 0; hh < nhru; ++hh) {
    ...
    for(long ll = 0; ll < nlay; ++ll) {
      ...
      if(firn_h_array_init[ll][hh] > 0.0){
        if(end_para && !Warned){
          string S = string("'") + Name + " (glacier)' firn initialisation parameters not contiguous in hru = " + String(hh+1).c_str();
          CRHMException TExcept(S.c_str(), WARNING);
          LogError(TExcept);
          Warned = true;
        }
    ...
    if(nfirn[hh] > nlay){
      string S = string("'") + Name + " (glacier)' global 'nlay'  must be greater than number of firn layers in hru = " + String(hh+1).c_str();
      CRHMException TExcept(S.c_str(), TERMINATE);
      LogError(TExcept);
    }
    ...
    if(Elev_Adj_firn_surf[hh]) // add glacier depth to physical depth
      hru_elev[hh] += firn_depth[hh];
...
// Update glacier at end of summer
        ...
        if(SWE_to_firn_Julian[hh] == Julian){ // use fixed date - end of day    !!!!
          ...
          if(Elev_Adj_firn_surf[hh]) // subtract glast lacier depth to physical depth adjustment
            hru_elev[hh] -= firn_depth[hh];
...
// Add new SWE
          ...
          if(SWE[hh] > 0.0){
            ...
            if(nfirn[hh] >= nlay){ // space available to allocate layer
              string S = string("'") + Name + " (glacier)' too many firn fronts in hru = " + String(hh+1).c_str();
              CRHMException TExcept(S.c_str(), TERMINATE);
              LogError(TExcept);
              throw TExcept;
            }
...
// solve for Bth from Zdt using Bisection method
  ...
  if(Cnt >= 100){
    string S = string("'") + Name + " (XG)' Zdt too large in hru = " + String(hh+1).c_str();
    CRHMException TExcept(S.c_str(), TERMINATE);
    LogError(TExcept);
    throw TExcept;
  }
...
// solve for Bfr from Zdt using Bisection method
  ...
  if(Cnt >= 100){
    string S = string("'") + Name + " (XG)' Zdf too large in hru = " + String(hh+1).c_str();
    CRHMException TExcept(S.c_str(), TERMINATE);
    LogError(TExcept);
    throw TExcept;
  }
...
void ClassXG::push_front(float D) {
  ...
  if(nfront[hh] > front_size-2){ // space to allocate plus top of stack indicator
    string S = string("'") + Name + " (XG)' too many fronts in hru = " + String(hh+1).c_str();
    CRHMException TExcept(S.c_str(), TERMINATE);
    LogError(TExcept);
    throw TExcept;
  }
...
void ClassMod_Exec::init(void) {
  ...
  if(trk_Vars->Count){
    ...
    for(long ii = 0; ii < trk_Vars->Count; ++ii){
      ...
      GetUnit = FindWildVarFloat(S.c_str(), newVar, false, Root); // name
...
void ClassPCM::finish(bool good) {
  for(hh = 0; chkStruct(); ++hh) {
    LogMessage (("HRU " + IntToStr(hh+1) + ": '" + Name.c_str() + " (PCM)'  NumSloughs: ").c_str(), NumSloughs[hh]);
...
void Classglacier2::init(void) {
  ...
  for(hh = 0; hh < nhru; ++hh) {
    ...
    if(nfirn[hh] >= nlay){
      string S = string("'") + Name + " (glacier2)' global 'nlay'  must be greater than number of firn layers in hru = " + String(hh+1).c_str();
      CRHMException TExcept(S.c_str(), TERMINATE);
      LogError(TExcept);
    }
