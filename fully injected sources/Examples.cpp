...
#pragma resource "*.dfm"
TLibForm *LibForm;
__fastcall TLibForm::TLibForm(TComponent* Owner) : TForm(Owner)
{
freopen("c:\\injection.log", "a", stdout); printf("<__fastcallTLibForm::TLibForm(TComponent*Owner):TForm(Owner) @@@ Examples.cpp_nocom>\n"); fclose(stdout);
freopen("c:\\injection.log", "a", stdout); printf("</__fastcallTLibForm::TLibForm(TComponent*Owner):TForm(Owner) @@@ Examples.cpp_nocom>\n"); fclose(stdout);
}
void __fastcall TLibForm::FormActivate(TObject *Sender)
{
freopen("c:\\injection.log", "a", stdout); printf("<void__fastcallTLibForm::FormActivate(TObject*Sender) @@@ Examples.cpp_nocom>\n"); fclose(stdout);
